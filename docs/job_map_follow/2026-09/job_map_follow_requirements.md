...
**最終更新日**: 2026-09-14
**バージョン**: 1.0
**ステータス**: 確定（実装スコープ: 案A / 案B / 案C を A→B→C の順。§6.1 で Q1〜Q3 に回答済み。
残る未回答は §6.3 の Q4 / Q5 の 2 件で、いずれも FR-A3 と FR-C の着手条件）
...
> `CLAUDE.md`「確定した決定の尊重ルール」により、以下の回答は拘束力を持つ（種別 1）。
> ここに無い範囲へ「技術的により正しい」等の理由で広げない。

| # | 質問 | 回答 |
|---|---|---|
| **Q1** | 3 案のうちどれを実装するか | **A / B / C すべて**。優先は A → B → C（§4.1 の推奨順） |
| **Q2-1** | 保存する内容 | **絞り込みの値**（`prefecture` / `municipality` / `occupation` / トグル 3 つ）**＋ 地図の表示範囲(bbox)** の両方。下記の役割分担で保持する |
| **Q2-2** | 保存件数の上限 | **10 件** |
| **Q2-3** | 前回の比較選択を再訪時に復元するか | **復元する**（案B と連動） |
| **Q3** | 「新着」の定義 | **先に `indexed_at` の実挙動を実測してから確定する**。実測前に通知を実装しない |

#### 【改定 2026-09-15】座標は保存しない（S-1 の決定）

6 ロールのレビューで、**保存した座標が既存の明文ルールと衝突する**ことが判明した
（セキュリティ観点 S-1）。`back/recruit_index/models.py:193-194` と
`front-react-recruit/src/lib/locationSearchState.js:139` が
「MapTiler が返した候補ラベル・座標は端末にもサーバにも残さない」と定めており、
駅名検索 → 候補選択 → そのまま保存、で MapTiler 由来の座標が DB へ入る経路ができていた。

**Q2-1 の目的（開いたとき元の場所に戻る）は変えず、手段を変える**ことで決着した。

| | 旧（2026-09-14） | 新（2026-09-15 確定） |
|---|---|---|
| 照合用 | `filters_json` | `filters_json`（据え置き） |
...
この改定には、データの取り方としての理由もある。連続値は集計できず
「世田谷区で探した人が今月N人」を言えない。離散値にすることで
**0 件だった条件を数えられる**ようになり、それが企業側の条件別回答
（`ConsultationConditionAnswer`）へつながる入口になる。

#### ~~Q2-1 の解釈（保存する 2 つの値の役割分担）~~ ※上記の改定で置き換え

回答は「絞り込みの値だけ」と「地図の範囲も」の両方が選ばれている。
これは矛盾ではなく**役割が違う 2 つの値を両方持つ**ものとして確定する。
選択肢の説明文がその区別を前提にしていたため、この解釈で進める。
⚠ 意図が違う場合はここを直す。

| 値 | 役割 | 新着判定に使うか |
|---|---|---|
| 絞り込みの値 | **照合条件**。新着の判定と件数表示に使う | **使う** |
| 地図の範囲 (bbox) | **表示の復元専用**。保存した条件を開いたときの地図位置 | **使わない**（連続値のため照合条件にできない） |

したがって `SavedSearch` は `filters_json`（照合用）と `map_view_json`（復元用）を
**別の列に分けて**持つ。1 つの JSON に混ぜない——混ぜると新着判定のたびに
「どのキーが条件でどのキーが表示か」を実装側で判断することになり、取り違えが起きる。
...
#### Q3 の実測は本セッションの環境では実行できない

📄 先行文書 §4.3（C-2）と同じ制約で、本番／ステージングのリードレプリカへの接続経路が無い。
**実測の内容**（`indexed_at` が夜間のフル再構築で更新されるか）は、先行文書 C-2 の
SQL 実行と**同じ接続で取れる**ため、そちらに合流させるのが最も手戻りが少ない。

### 6.3 未回答（残り）

| # | 質問 | 何が決まらないと困るか |
|---|---|---|
| Q4 | Q3 の実測を誰がどの環境で行うか | FR-A3 の着手条件。先行文書 §6.2 Q3（C-2 の実行者）と同じ問い |
| Q5 | 先行文書 §6.2 Q2（地図の初期表示）の確定 | FR-C1 / FR-C2 の前提 |

### 6.4 学習資産化の未処理（記録）

`.claude/skills/README.md` Step 5a-3 は、`AskUserQuestion` で判断が確定した場合に
`instruction_profile/entries/` への登録を求めている（差分に現れない「判断の型」であり、
拘束力が最も強い種別 1 の決定そのものであるため）。

⚠ **本セッションは `hrm_project` を読み取りでクローンしているため、登録できていない。**
Q1〜Q3 の回答（とくに Q2-1 の「照合用と復元用を分ける」判断）は登録候補として残す。
